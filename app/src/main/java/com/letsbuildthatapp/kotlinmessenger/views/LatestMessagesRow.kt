    ...
    override fun bind(viewHolder: ViewHolder, position: Int) {
        ...
        ref.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(p0: DataSnapshot) {
                chatPartherUser = p0.getValue(User::class.java)

                //viewHolder.itemView.username_textview_latest_message.text = "USERNAME"
                viewHolder.itemView.username_textview_latest_message.text = chatPartherUser?.username

                val targetImageView = viewHolder.itemView.imageview_latest_message
                Picasso.get().load(chatPartherUser?.profileImageUrl).into(targetImageView)
            }
